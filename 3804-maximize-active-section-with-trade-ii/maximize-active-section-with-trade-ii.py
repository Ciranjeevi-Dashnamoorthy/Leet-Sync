"""
Use seg tree for efficinet querying of min-max 
similar logic of trade 1
"""
class ST:
    ...
    def query(self, node, l, r, ql, qr):
        ...
        if ql <= l and r <= qr: return self.st[node]
        mid = l + (r - l) // 2
...
class Solution:
    def maxActiveSectionsAfterTrade(self, s: str, queries: List[List[int]]) -> List[int]:
        ...
        if len(zeros) < 2: return [ones] * len(queries)
        ...
        for l, r in queries:
            ...
            prevl = min(zeros[first][1], r) - max(zeros[first][0], l) + 1
            nextl = min(zeros[first + 1][1], r) - max(zeros[first + 1][0], l) + 1
            best = max(best, prevl + nextl)

            prevl = min(zeros[last - 1][1], r) - max(zeros[last - 1][0], l) + 1
            nextl = min(zeros[last][1], r) - max(zeros[last][0], l) + 1
            best = max(best, prevl + nextl)
